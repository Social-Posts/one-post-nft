...
const SellModal: React.FC<SellModalProps> = ({
  ...
  const handleSell = async () => {
    if (!post || !address || !price || !walletClient) return;
    ...
    try {
      ...
      const txHash = await proposeSell(
        walletClient,
        post.tokenId,
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-muted/30 rounded-lg p-3">
            <p className="text-xs text-muted-foreground">
              💡 <strong>Note:</strong> Once listed, your NFT will appear in the
              marketplace. You can cancel the listing anytime before someone
              buys it.
            </p>
          </div>
        </div>
