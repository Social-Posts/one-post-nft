...
const BuyModal: React.FC<BuyModalProps> = ({ isOpen, onClose, post }) => {
  ...
  const handlePurchase = async () => {
    ...
    if (
      ...
      selectedPaymentMethod !== "mockbase"
    ) {
      showErrorToast(
        "Unsupported payment method",
        "Only ETH and MockBASE payments are currently supported"
      );
      return;
    }
    ...
    try {
      ...
      if (selectedPaymentMethod === "mockbase") {
        ...
        const CONTRACT_ADDRESS = deployedContracts.base.OnePostNFT
        ...
        loadingToast = showLoadingToast("🔓 Approving MockBASE tokens...");
        await approveERC20(
          walletClient,
          CONTRACT_ADDRESS,
          BigInt(Math.floor(post.price * 1e18))
        );

        // Then buy the NFT with MockBASE
        if (loadingToast) dismissToast(loadingToast);
        loadingToast = showLoadingToast("💳 Processing MockBASE purchase...");
      ...
      if (loadingToast) dismissToast(loadingToast);
      showSuccessToast(`🎉 Successfully purchased NFT #${post.tokenId}! 🚀`);
      ...
      showErrorToast(`❌ Failed to purchase NFT #${post.tokenId}`, error);
    } finally {
        ...
        <div className="space-y-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="secondary">
                    {formatEther(BigInt(post.price.toString()))} ETH
                  </Badge>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between font-medium">
                ...
                <span>
                  {(
                    parseFloat(formatEther(BigInt(post.price.toString()))) +
                    0.001
                  ).toFixed(4)}{" "}
                  ETH
                </span>
          ...
          <div className="flex gap-3">
            ...
            >
              {isProcessing
                ? "Processing..."
                : `Buy for ${formatEther(BigInt(post.price.toString()))} ${
                    selectedPaymentMethod === "mockbase" ? "MockBASE" : "ETH"
                  }`}
            </Button>
          </div>
