...
const BrowseSwaps: React.FC = () => {
  ...
  const { address } = useAccount(); // For wallet detection
  ...
  const handleCancelSell = async (post: Post) => {
    ...
    try {
      ...
      const txHash = await cancelSell(walletClient, post.tokenId);
      toast.dismiss();
      ...
      const load = async () => {
        ...
        try {
          const posts = await getAllPosts(0, FEED_PAGE_SIZE * 2);
          const forSale = posts.filter(p => p.isForSale);
          setSwappablePosts(forSale);
        } catch (error) {
  ...
  const handleChat = (post: Post) => {
    ...
    // Navigate to chat page - for now just show success message since onNavigate is not available
