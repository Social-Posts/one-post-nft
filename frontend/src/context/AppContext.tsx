      ...
      try {
        ...
        if (address) {
          const [canPost, userPosts] = await Promise.all([
            canUserPostToday(address as `0x${string}`),
            getUserPosts(address as `0x${string}`),
          ]);
...
// return;
      await createPostOnChain(walletClient, contentHash, 0);
  ...
  const refreshUserData = async () => {
    if (!address) return;
    const userPosts = await getUserPosts(address as `0x${string}`);
    dispatch({ type: 'SET_USER_POSTS', payload: userPosts });
  };
