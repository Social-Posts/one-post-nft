...
export class ChatService {
  ...
  static async getOrCreateChat(
    ...
    const [p1, p2] = [
      participant1.toLowerCase(),
      participant2.toLowerCase(),
    ].sort();
    ...
    const chatUsers = await Promise.all(
      chats.map(async (chat) => {
        const otherParticipant =
          chat.participant_1.toLowerCase() === userAddress.toLowerCase()
            ? chat.participant_2
            : chat.participant_1;
  ...
  static async getTotalUnreadCount(userAddress: string): Promise<number> {
    ...
    const { data: chats, error } = await supabase
      ...
      .select("id")
      .or(
        `participant_1.eq.${userAddress.toLowerCase()},participant_2.eq.${userAddress.toLowerCase()}`
      );
  ...
  static async sendMessage(
    ...
    try {
      const senderName = `User ${senderAddress.slice(-3)}.base.eth`;
  ...
  // Mark messages as read (duplicate method removed)
...
export class LikesService {
  ...
  static async addLike(
    ...
    if (
      ...
      try {
        const likerName = `User ${userAddress.slice(-3)}.base.eth`;
