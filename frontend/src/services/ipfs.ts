...
export async function storeOnIPFS(metadata: Record<string, unknown>): Promise<string> {
  ...
  // Extend the global Window type to include an optional PINATA_JWT so we don't use `any`.
  declare global {
    interface Window {
      PINATA_JWT?: string;
    }
  ...
  const runtimeJwt = (globalThis as unknown as Window).PINATA_JWT || localStorage.getItem('PINATA_JWT');
